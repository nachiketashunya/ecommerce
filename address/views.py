...
#view function to create checkout address
def checkout_address(request):
	addressform = AddressForm(request.POST or None )  #initializing address form
...
#View function to reuse saved address
def checkout_address_reuse(request):
	if 'username' in request.session or 'guest_email_id' in request.session:
		...
		if request.method == "POST":
			...
			if shipping_address is not None:	#This means user clicked on radio button
				...
				if is_safe_url(redirect_path , request.get_host()):	#Checking for safe url
					return redirect(redirect_path)
				else:
					return redirect('/cart/checkout')
